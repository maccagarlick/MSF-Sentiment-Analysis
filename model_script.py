...
This script also contains functions to extract the embeddings data to perform attention weight analysis on the model outputs.
"""
    ...
    for file in os.listdir(individual_files_directory):
        ...
        while attempts < 3:
            ...
            for i in range(current_data_abbriv.shape[0]):
                ...
                # Create enhanced prompt for better content focus
                ...
                cleaned_text, embeddings, attentions, inputs = generate(model, tokenizer, prompt, current_data)
                
                # Save the attention, embeddings and token data for individual responses
                tokens = tokenizer.convert_ids_to_tokens(inputs['input_ids'][0])
                
                # Save analysis data (embeddings, attention weights, and tokens)
                if embeddings is not None:
                    save_analysis_data(embeddings, attentions, tokens, RC, working_directory)
        ...
        if len(all_responses) < len(current_data):
            ...
            while retry_attempts < 3 and len(all_responses) < len(current_data):
                ...
                for i in range(missing_data.shape[0]):
                    ...
                    # Create enhanced retry prompt for better content focus
                    data_prompt = f"Response Code: {RC}\nMT Comments: {MT}\nVC Comments: {VC}\nTW Comments: {TW}\nA Comments: {A}\n"
                    prompt = f"STUDENT FEEDBACK:\n{data_prompt}\n{system_prompt}\n\nFor Response Code {RC}, output EXACTLY: [{RC}, MT_score, VC_score, TW_score, A_score]"
                    
                    print(f"Retrying for response code {RC}")

                    cleaned_text, missing_embeddings, missing_attentions, missing_inputs = generate(model, tokenizer, prompt, current_data)
                    
                    # Save the attention, embeddings and token data for retry attempts
                    missing_tokens = tokenizer.convert_ids_to_tokens(missing_inputs['input_ids'][0])
                    
                    # Save analysis data (embeddings, attention weights, and tokens)
                    if missing_embeddings is not None:
                        save_analysis_data(missing_embeddings, missing_attentions, missing_tokens, RC, working_directory)
...
def generate(model, tokenizer, prompt, current_data):
    ...
    2. Extracts input embeddings for analysis
    3. Generates a response from the model using conservative parameters for consistency
    4. Extracts attention weights from the generation process
    5. Removes the input prompt from the output text and cleans the response
    6. Returns the cleaned response text along with analysis data
    ...
    Returns:
        tuple: A tuple containing:
            - cleaned_text (str): The cleaned response text generated by the model
            - input_embeddings (torch.Tensor): The input embeddings from the model
            - attentions (tuple/list or None): Attention weights from model layers
            - inputs (dict): The tokenised inputs used for generation
    """
    ...
    # Extract input embeddings before generation (with gradients for analysis)
    input_embeddings = model.model.embed_tokens(inputs['input_ids'])
    ...
    attentions = None
    if hasattr(response, 'attentions') and response.attentions:
        # Extract attentions from the last response step
        if len(response.attentions) > 0:
            attentions = response.attentions[-1]
    
    # Get the response text and remove the input prompt
    input_len = inputs['input_ids'].shape[1]
    
    # Debug: Check if response was generated
    if hasattr(response, 'sequences'):
        generated_tokens = response.sequences[0][input_len:]
        print(f"Debug: Generated {len(generated_tokens)} new tokens")
        if len(generated_tokens) == 0:
            print("Debug: No new tokens generated - model may have stopped immediately")
    ...
    if len(response_text) == 0:
        print("Debug: Empty response from model")
    ...
    return cleaned_text, input_embeddings, attentions, inputs
...
def save_analysis_data(embeddings, attentions, tokens, response_code, working_directory):
    """
    Save embeddings, attention weights, and tokens for analysis with response code-prefixed file names.
    
    Args:
        embeddings (torch.Tensor): The input embeddings from the model.
        attentions (tuple/list or None): Attention weights from model layers.
        tokens (list): List of tokenised input tokens.
        response_code (int): The response code number for file naming.
        working_directory (str): The working directory path.
    ...
    analysis_dir = os.path.join(working_directory, "data", "analysis")
    os.makedirs(analysis_dir, exist_ok=True)
    
    # Save embeddings with response code prefix
    embeddings_path = os.path.join(analysis_dir, f"{response_code}_embeddings.pt")
    torch.save(embeddings.cpu(), embeddings_path)
    
    # Save attention weights with response code prefix
    if attentions is not None:
        attention_path = os.path.join(analysis_dir, f"{response_code}_attentions.pt")
        try:
            if isinstance(attentions, (tuple, list)) and len(attentions) > 0:
                attention_tensors = []
                for i, layer_attention in enumerate(attentions):
                    if hasattr(layer_attention, 'cpu'):
                        attention_tensors.append(layer_attention.cpu())
                
                if attention_tensors:
                    torch.save(attention_tensors, attention_path)
                    print(f"Saved {len(attention_tensors)} attention layers to {analysis_dir}")
                else:
                    print("No valid attention tensors found")
            else:
                print("Attentions is empty or not a valid structure")
                
        except Exception as e:
            print(f"Warning: Could not save attention weights: {e}")
            print(f"Attention structure type: {type(attentions)}")
    else:
        print("No attention weights to save (attentions is None)")
    
    # Save tokens for reference with response code prefix
    tokens_path = os.path.join(analysis_dir, f"{response_code}_tokens.txt")
    with open(tokens_path, 'w') as f:
        f.write('\n'.join([str(i) + ': ' + token for i, token in enumerate(tokens)]))
